...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !use_token_cache {
        ...
        for swap in &swaps {
            all_mints.push(swap.sold_mint.clone());
            all_mints.push(swap.bought_mint.clone());
        }

        let unique_mints: HashSet<_> = all_mints.iter().cloned().collect();
